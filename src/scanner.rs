			...
			_ => {
				if c.is_ascii_digit() {
					...
					let kind: TokenType = match i.readIdentifier().as_str() {
						"and" => i.reserved("and", "'and' operators in LunarSharp are made with '&&'"),
						"not" => i.reserved("not", "'not' operators in LunarSharp are made with '!'"),
						"or" => i.reserved("or", "'or' operators in LunarSharp are made with '||'"),
						"do" => i.reserved("do", "'do ... end' blocks in LunarSharp are made like this: '{ ... }'"),
						"end" => i.reserved("end", "code blocks in LunarSharp are closed with '}'"),
						"repeat" => i.reserved("repeat", "'repeat ... until x' loops in LunarSharp are made like this: 'loop { ... } until x'"),
						"then" => i.reserved("then", "code blocks in LunarSharp are opened with '{'"),
						"if" => IF,
						"elseif" => ELSEIF,
						"else" => ELSE,
						"for" => FOR,
						...
						"while" => WHILE,
						"until" => UNTIL,
						"local" => i.reserved("local", "current class only variables are made using `protected` in LunarSharp"),
						"return" => RETURN,
						"true" => TRUE,
						"false" => FALSE,
						"nil" => NIL,
						...
						"match" => MATCH,
						"default" => DEFAULT,
						"macro" => i.reserved("macro", "macros are defined using `@define` in LunarSharp"),
